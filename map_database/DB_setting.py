...
# 데이터베이스 연결 정보
database_info = open('/docker-entrypoint-initdb.d/database_id.txt', 'r').readlines()
...
# API KEY
API_key = open('/docker-entrypoint-initdb.d/api_key.txt', 'r').readlines()
...
# 데이터 생성 함수
def create_user_data(stock_df, num_users=10):
    # tb_user_key
    ...
    details = {
        'income': ['월급', '선물', '이자', '임대', '보너스', '환급', '기타'],
        'expense': ['임대료', '식료품', '공과금', '교통', '식사', '엔터테인먼트', '건강관리']
    }
    fixed_incomes = ['월급', '보너스', '환급']
    fixed_expenses = ['임대료', '공과금', '교통']
    ...
    for user_id in user_ids:
        ...
        while current_date <= end_date:
            ...
            for detail in details['income']:
                ...
                records.append([user_id, current_date, detail, amount, 0, 0])
            ...
            for detail in details['expense']:
                ...
                records.append([user_id, current_date, detail, amount, 0, 1])
    ...
    shares_dates = stock_df['fd_date'].tolist()  # stock_df에서 fd_date 리스트를 가져옴

    share_records = []
    for user_id in user_ids:
        for i, date in enumerate(shares_dates):
            if i == 0:  # 첫날은 비교할 데이터가 없으므로 건너뜀
                continue
            
            # 해당 날짜의 주가 변화량
            ss_change = stock_df.loc[i, 'sc_ss_stock'] - stock_df.loc[i - 1, 'sc_ss_stock']
            ap_change = stock_df.loc[i, 'sc_ap_stock'] - stock_df.loc[i - 1, 'sc_ap_stock']
            bit_change = stock_df.loc[i, 'sc_coin'] - stock_df.loc[i - 1, 'sc_coin']

            # 주가가 올랐으면 매수(양수), 내렸으면 매도(음수)
            ss_count = np.random.randint(1, 10) if ss_change > 0 else np.random.randint(-10, 0)
            ap_count = np.random.randint(1, 10) if ap_change > 0 else np.random.randint(-10, 0)
            bit_count = np.random.randint(1, 10) if bit_change > 0 else np.random.randint(-10, 0)
...
df_user_key, df_user_information, df_user_finance, df_received_paid, df_shares_held = create_user_data(stock_df)
