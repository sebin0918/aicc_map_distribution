...
const deleteId = async (req, res) => {
  ...
  console.log("세션에 저장된 이메일:", userEmail);
  console.log("세션에 저장된 사용자 ID:", userId);

  // 세션에 이메일이 없으면 userId로 조회
  if (!userEmail) {
    if (!userId) {
    ...
    try {
      ...
      connection = await pool.getConnection();
      const query = "SELECT uk_email FROM tb_user_key WHERE user_id = ?";
      const [rows] = await connection.execute(query, [userId]); // `rows`만 받아옴

      // `rows`가 객체인지 확인하고 배열로 변환
      console.log('쿼리 실행 결과 (rows):', rows); // 쿼리 결과를 로그로 확인
      const resultArray = Array.isArray(rows) ? rows : [rows]; // 배열로 변환

      if (!resultArray || resultArray.length === 0) {  // rows가 배열인지 확인
        console.error('사용자를 찾을 수 없습니다.');
        return res.status(404).json({ message: '사용자를 찾을 수 없습니다.' });
      }

      userEmail = resultArray[0].uk_email;  // 배열에서 첫 번째 결과를 사용
      console.log("조회된 이메일:", userEmail);
    } catch (err) {
      console.error('사용자 이메일 조회 중 에러:', err);
  ...
  if (userEmail) {
    let connection;
    try {
      console.log('회원 삭제 쿼리 실행 중...');
      connection = await pool.getConnection();
      const query = "DELETE FROM tb_user_key WHERE uk_email = ?";
      // DELETE 쿼리 실행 결과는 항상 [result, fields] 형태로 반환됩니다.
      const results = await connection.execute(query, [userEmail]); 

      console.log("회원삭제 쿼리 결과:", results);

      // 결과 확인 후 로그아웃 처리
      if (results.affectedRows > 0) { // 탈퇴 성공 확인
    ...
    console.error('이메일이 조회되지 않았습니다.');
    res.status(500).json({ message: '서버 에러: 이메일이 조회되지 않았습니다.' });
  }
};
