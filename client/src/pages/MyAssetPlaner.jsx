...
const MyAssetPlaner = () => {
  ...
  const fetchTotalAssetData = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/my-asset-planer/total-asset', { credentials: 'include' });
  ...
  const fetchCurrentMonthReceived = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/my-asset-planer/current-month-received', { credentials: 'include' });
      const data = await response.json();
      if (response.ok) {
  ...
  const fetchMonthlyExpenditures = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/my-asset-planer/monthly-expenditures', { credentials: 'include' });
      const data = await response.json();
      if (response.ok) {
  ...
  useEffect(() => {
    const checkBankAccount = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/my-asset-planer/check-bank-account', { credentials: 'include' });
        const data = await response.json();
  ...
  useEffect(() => {
    if (hasBankAccount === true) {
      fetchData('http://localhost:5000/api/my-asset-planer/deposit', (data) => setDeposit(parseFloat(data.userDeposit) || 0));
      fetchData('http://localhost:5000/api/my-asset-planer/installmentsaving', (data) => setInstallmentSaving(parseFloat(data.userInstallmentSaving) || 0));
      fetchData('http://localhost:5000/api/my-asset-planer/loan', (data) => setUserLoan(parseFloat(data.userLoan) || 0));
      fetchData('http://localhost:5000/api/my-asset-planer/stock', (data) => {
        setSamsungStock(data.samsung || {});
        setAppleStock(data.apple || {});
        setCoin(data.coin || {});
        setTotal(data.total || {});
      });
      fetchData('http://localhost:5000/api/my-asset-planer/target', (data) => setCurrentBudget(data.targetBudget ? data.targetBudget.toLocaleString() : "0"));
      fetchTotalAssetData();
    }
  }, [hasBankAccount]);  
  ...
  const Modal = ({ isOpen, onClose }) => {
    ...
    useEffect(() => {
      ...
      const fetchInvestments = async () => {
        try {
          const response = await fetch("http://localhost:5000/api/my-asset-planer/get-investments", {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
            },
    ...
    const handleSave = async () => {
      ...
      // 서버에 데이터 전송
      try {
        const response = await fetch("http://localhost:5000/api/my-asset-planer/save-investments", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
